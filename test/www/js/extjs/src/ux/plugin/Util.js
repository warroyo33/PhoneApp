...
Ext.define("Ext.ux.plugin.Util",{
    ...
    syncContent : function(url,nocache,response,params) {
        ...
        if(typeof params == "object") url  += (url.indexOf('?') != -1 ? '&' : '?') + Ext.toQueryString(params);
        try {
    ...
    formObject : function(form){
        ...
        } else if (form instanceof Ext.form.Panel) {
            form = form.getForm().getEl().dom;
        } else if (form instanceof Ext.form.Basic) {
            form = form.getEl().dom;
        }
    ...
    require : function(packages,options){
        var _require = Ext.bind(function(packages,options){
            var ret = {css: [],js:[]};
